...
**Implemented cards (`922100148`–`922100171`):** use [`black_saints.md`](black_saints.md) as the source of truth (text from `expansions/saint-seiya.cdb`, plus Lua header blocks from `script/unofficial/`). Regenerate that file with `python sets/saint_seiya/export_black_saints_md_from_cdb.py`.

Este documento conserva el borrador PSCT en ingles para el arquetipo **Black Saints** y cartas relacionadas; puede desactualizarse respecto al `.cdb` y a los `.lua`.
