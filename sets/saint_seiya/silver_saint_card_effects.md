# Silver Saints - Pope's Mandate (PSCT)

Documento para una variante **deck solo Silver** de enfoque **Control/Opresion**, inspirada en la saga del Santuario cuando los Silver Saints obedecen al Patriarca.

En esta version, los Silver Saints son **Effect Monsters de Main Deck** (no Synchro) y llevan el tag **"Envoy of the Pope"** en su nombre, identificandolos como parte del arquetipo del Pope. Esto los diferencia de las versiones Synchro del deck Bronze+Silver (`saint_cards_effects.md`), que comparten nombre de personaje y constelacion pero **no** el tag de envoy. El soporte central son las cartas "Pope's Mandate" y el boss final es `Pope Ares`.

---

## Core Rule del Subarquetipo

- Todos los monstruos llevan el tag `Envoy of the Pope` como marcador de arquetipo (usable por soporte Pope para busqueda/lock).
- Casi todos los Silver Saints comparten esta clausula:
  - "If you control 'Pope Ares' or a 'Pope's Mandate' card, you can Special Summon this card from your hand."
- Esto permite jugarlos sin Bronze Saints y sostener identidad de "escuadron represivo".

---

## Command Unit

### Pope Ares - Voice of the Sanctuary
- **Card Type**: Effect Monster
- **Attribute**: DARK
- **Type**: Spellcaster
- **Level**: 4
- **ATK/DEF**: 1000 / 1800

```text
If this card is Normal or Special Summoned: You can add 1 "Pope's Mandate" card from your Deck to your hand, except "Pope Ares - Voice of the Sanctuary".
While this card is face-up on the field, your opponent cannot target other "Envoy of the Pope" monsters you control with card effects.
If this card is sent from the field to the GY by an opponent's card: You can Special Summon 1 "Envoy of the Pope" monster from your hand or GY, except "Pope Ares - Voice of the Sanctuary".
During your Main Phase: You can banish 1 "Envoy of the Pope" monster from your GY; Special Summon this card from your GY.
You can only use each effect of "Pope Ares - Voice of the Sanctuary" once per turn.
```

---

## Silver Saints (Effect Versions)

### Silver Saint - Marin of Eagle, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: WIND
- **Type**: Warrior
- **Level**: 6
- **ATK/DEF**: 2200 / 1400

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
If this card is Normal or Special Summoned: You can add 1 "Pope's Mandate" Spell/Trap from your Deck to your hand, except "Pope's Mandate - Chain of Command".
(Quick Effect): You can target 1 face-up monster your opponent controls; until the end of this turn, it loses 800 ATK, also its effects are negated.
You can only use each effect of "Silver Saint - Marin of Eagle, Envoy of the Pope" once per turn.
```

### Silver Saint - Shaina of Ophiuchus, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: LIGHT
- **Type**: Warrior
- **Level**: 6
- **ATK/DEF**: 2400 / 1200

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
Once per turn: You can pay 800 LP, then target 1 face-up monster your opponent controls; its effects are negated until the end of this turn.
If you control "Pope Ares", this card can make a second attack during each Battle Phase.
```

### Silver Saint - Algol of Perseus, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: EARTH
- **Type**: Warrior
- **Level**: 6
- **ATK/DEF**: 2300 / 1800

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
Once per turn: You can target 1 face-up monster your opponent controls; change it to Defense Position, and if you do, it cannot change its battle position, also its effects are negated, until the end of your opponent's next turn.
Monsters your opponent controls in this card's column cannot activate their effects.
```

### Silver Saint - Misty of Lacerta, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: WATER
- **Type**: Warrior
- **Level**: 6
- **ATK/DEF**: 2000 / 2500

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
Your opponent cannot target other "Envoy of the Pope" monsters you control with card effects.
Once per turn, if this card would be destroyed by battle or card effect, it is not destroyed.
If this face-up card leaves the field by an opponent's card: You can Set 1 "Pope's Mandate" Spell/Trap directly from your Deck.
```

### Silver Saint - Orphee of Lyra, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: LIGHT
- **Type**: Warrior
- **Level**: 7
- **ATK/DEF**: 2700 / 2000

```text
Cannot be Normal Summoned/Set.
Must be Special Summoned (from your hand) by controlling 2 or more "Envoy of the Pope" monsters.
Once per turn (Quick Effect): You can send 1 card from your hand or face-up field to the GY; negate the effects of all face-up monsters your opponent currently controls, until the end of this turn.
If you control "Pope Ares", your opponent cannot activate monster effects in response to this effect's activation.
```

### Silver Saint - Hound Asterion, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: LIGHT
- **Type**: Warrior
- **Level**: 6
- **ATK/DEF**: 2400 / 2000

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
Once per turn: You can reveal 1 random card in your opponent's hand, then apply this effect based on its type.
● Monster: Negate the effects of 1 face-up monster your opponent controls until the end of this turn.
● Spell/Trap: Set 1 Spell/Trap your opponent controls face-down.
If you control "Pope Ares", your opponent cannot activate cards or effects with the same original name as the revealed card until the end of this turn.
```

### Silver Saint - Whale Moses, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: WATER
- **Type**: Warrior
- **Level**: 6
- **ATK/DEF**: 2500 / 2200

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
Once per turn (Quick Effect): You can target 1 face-up monster your opponent controls; return it to the hand.
Also, your opponent cannot Special Summon monsters with that original name from the Extra Deck until the end of their next turn.
```

### Silver Saint - Centaurus Babel, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: WIND
- **Type**: Warrior
- **Level**: 6
- **ATK/DEF**: 2300 / 2100

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
When your opponent activates a Spell/Trap Card or effect (Quick Effect): You can send 1 "Envoy of the Pope" card from your hand or face-up field to the GY; negate that activation, and if you do, destroy that card.
You can only use this effect of "Silver Saint - Centaurus Babel, Envoy of the Pope" once per turn.
```

### Silver Saint - Crow Jamian, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: DARK
- **Type**: Warrior
- **Level**: 6
- **ATK/DEF**: 2400 / 1800

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
Once per turn: You can target 1 face-up monster your opponent controls; it loses 800 ATK, also it cannot attack or activate its effects this turn.
If that targeted monster leaves the field this turn: Inflict 600 damage to your opponent.
```

### Silver Saint - Cerberus Dante, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: DARK
- **Type**: Warrior
- **Level**: 6
- **ATK/DEF**: 2500 / 2300

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
Once per turn: You can target up to 2 cards in either GY; banish them.
If a card(s) is banished by this effect, your opponent cannot activate cards or effects with the same original name as those banished cards until the end of this turn.
```

### Silver Saint - Auriga Capella, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: LIGHT
- **Type**: Warrior
- **Level**: 6
- **ATK/DEF**: 2600 / 1900

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
Once per turn (Quick Effect): You can target 1 face-up Spell/Trap your opponent controls; destroy it, then if you control another "Envoy of the Pope" monster, this card gains 400 ATK until the end of this turn.
```

### Silver Saint - Canis Major Sirius, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: EARTH
- **Type**: Warrior
- **Level**: 6
- **ATK/DEF**: 2500 / 2100

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
When this card battles an opponent's monster, at the start of the Damage Step: You can make that opponent's monster lose 1000 ATK/DEF until the end of this turn.
Once per turn, when your opponent activates a monster effect in the Battle Phase (Quick Effect): You can negate that effect.
```

### Silver Saint - Musca Dio, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: DARK
- **Type**: Warrior
- **Level**: 6
- **ATK/DEF**: 2200 / 2200

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
Once per turn: You can target 1 face-up monster your opponent controls; place 1 Fly Counter on it.
Monsters with a Fly Counter have their effects negated, also they cannot be used as material for a Special Summon from the Extra Deck.
```

### Silver Saint - Heracles Algethi, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: EARTH
- **Type**: Warrior
- **Level**: 7
- **ATK/DEF**: 2700 / 2000

```text
Cannot be Normal Summoned/Set.
Must be Special Summoned (from your hand) by banishing 1 "Envoy of the Pope" monster from your GY.
Once per turn (Quick Effect): You can target 1 face-up monster your opponent controls; its ATK becomes 0 until the end of this turn, also this card can make a second attack during each Battle Phase this turn.
```

### Silver Saint - Sagitta Ptolemy, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: LIGHT
- **Type**: Warrior
- **Level**: 6
- **ATK/DEF**: 2300 / 1700

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
Once per turn: You can target 1 face-up monster your opponent controls; inflict damage to your opponent equal to half that monster's current ATK, and if you do, that target cannot activate its effects this turn.
```

### Silver Saint - Cepheus Daidalos, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: LIGHT
- **Type**: Warrior
- **Level**: 6
- **ATK/DEF**: 2400 / 2400

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
If this card is Normal or Special Summoned: You can Special Summon 1 Level 4 or lower "Envoy of the Pope" monster from your GY, but negate its effects.
Once per turn (Quick Effect): You can target 1 face-up monster on the field; it cannot be destroyed by battle this turn, also it cannot activate its effects this turn.
```

---

## Ghost Saints (Low-Tier Envoys)

Sub-arquetipo **"Ghost Saint"** entrenado por Shaina bajo órdenes directas del Pope. Funcionan como **fodder / swarm / extenders** de bajo nivel para habilitar invocaciones Xyz/Link pequeñas y pagar costos de los envoys mayores. Comparten el tag `Envoy of the Pope` (archetype lock principal) más el sub-tag `Ghost Saint` (sinergia interna).

### Ghost Saint - Geist, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: DARK
- **Type**: Warrior
- **Level**: 4
- **ATK/DEF**: 1400 / 1400

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
If this card is Normal or Special Summoned: You can add 1 "Ghost Saint" monster from your Deck to your hand, except "Ghost Saint - Geist, Envoy of the Pope".
Once per turn, if a "Ghost Saint" monster(s) you control would be destroyed by battle or card effect, you can discard 1 "Envoy of the Pope" monster instead.
You can only use each effect of "Ghost Saint - Geist, Envoy of the Pope" once per turn.
```

### Ghost Saint - Astaroth, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: DARK
- **Type**: Fiend
- **Level**: 3
- **ATK/DEF**: 1200 / 800

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
If this card is Normal or Special Summoned: You can send 1 "Envoy of the Pope" card from your Deck to the GY.
(Quick Effect): You can banish this card from your GY, then target 1 Set card your opponent controls; your opponent reveals that card, and if it is a Spell/Trap, destroy it.
You can only use each effect of "Ghost Saint - Astaroth, Envoy of the Pope" once per turn.
```

### Ghost Saint - Iguana, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: EARTH
- **Type**: Reptile
- **Level**: 2
- **ATK/DEF**: 800 / 400

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
During your Main Phase: You can discard 1 "Envoy of the Pope" monster; Special Summon 1 Level 4 or lower "Ghost Saint" monster from your Deck, but its effects are negated.
If this card is sent to the GY as Link Material or Xyz Material: You can add 1 "Pope's Mandate" card from your GY to your hand.
You can only use each effect of "Ghost Saint - Iguana, Envoy of the Pope" once per turn.
```

---

## Apprentices of the Sanctuary (Mid-Tier Envoys)

Discípulos jóvenes adoctrinados directamente en el Santuario bajo órdenes del Pope. No son Silver Saints plenos (no portan Cloths plateados consagrados), pero ejecutan misiones específicas y sirven como **pegamento mecánico** entre los tiers bajos (Ghosts) y altos (Silvers/Gold). Incluye a los estudiantes más notables del anime: **Cassios** (alumno de Shaina), **Shiva de Pavo Real** y **Agora de Loto** (los dos discípulos indios de Shaka, asignados al servicio del Pope durante la saga del Santuario).

### Cassios, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: EARTH
- **Type**: Warrior
- **Level**: 4
- **ATK/DEF**: 1800 / 1400

```text
If you control "Pope Ares", a "Pope's Mandate" card, or a "Silver Saint" Envoy monster, you can Special Summon this card from your hand.
(Quick Effect): You can Tribute this card, then target 1 "Envoy of the Pope" monster you control; that monster cannot be destroyed by battle or card effect until the end of the next turn, also your opponent cannot target it with card effects during that period.
If this card is sent from the field to the GY as a cost or by an opponent's card: You can add 1 "Silver Saint - Shaina of Ophiuchus, Envoy of the Pope" or 1 "Pope's Mandate" card from your Deck or GY to your hand.
You can only use each effect of "Cassios, Envoy of the Pope" once per turn.
```

Flavor: Cassios, el alumno fiel de Shaina. Su efecto de tributarse refleja su sacrificio final por Seiya en el anime — aquí redirigido al Pope, protegiendo al Silver más cercano.

### Shiva of Peacock, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: LIGHT
- **Type**: Warrior
- **Level**: 5
- **ATK/DEF**: 1700 / 1600

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
This card can attack your opponent directly if your opponent controls no Effect Monsters.
Once per turn, when this card declares an attack: You can banish 1 "Envoy of the Pope" monster from your GY; your opponent cannot activate cards or effects in response to this attack.
If this card is in your GY: You can banish 1 other "Envoy of the Pope" monster from your GY; Special Summon this card.
You can only use each effect of "Shiva of Peacock, Envoy of the Pope" once per turn.
```

Flavor: Shiva, discípulo de Shaka, maestro del golpe a velocidad-luz. Su efecto de silencio durante ataque representa el "Ataque del Pavo Real" que no permite defensa ni contraataque.

### Agora of Lotus, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: DARK
- **Type**: Spellcaster
- **Level**: 5
- **ATK/DEF**: 1800 / 1500

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
Once per turn (Quick Effect): You can discard 1 "Envoy of the Pope" card, then target 1 face-up monster your opponent controls; that monster's effects are negated until your next Standby Phase.
While you control "Pope Ares", this card and other "Envoy of the Pope" monsters you control cannot be targeted by your opponent's Spellcaster-Type monster effects.
If this card is in your GY: You can send 1 "Pope's Mandate" Spell/Trap from your Deck to the GY; Special Summon this card.
You can only use each effect of "Agora of Lotus, Envoy of the Pope" once per turn.
```

Flavor: Agora, el otro discípulo de Shaka y contraparte mística de Shiva. Silencia los sentidos del rival — un eco del "Tesoro del Cielo" que su maestro emplearía en Virgo.

---

## Silver Assassins (Mid-Tier Mercenary Envoys)

Asesinos contratados directamente por el Pope fuera del linaje oficial de Silver Saints. Más brutos, más desechables, y perfectos como **beaters mid-tier** con utilidad puntual. No portan constelación en el nombre porque técnicamente **no son Saints** — son armas humanas entrenadas para misiones especiales (como asesinar a los Bronze Saints antes de que lleguen al Santuario).

### Docrates, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: EARTH
- **Type**: Warrior
- **Level**: 6
- **ATK/DEF**: 2400 / 1600

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Normal Summon this card without Tributing, but its ATK becomes 1800 until the End Phase.
If this card is Normal or Special Summoned: You can destroy 1 Spell/Trap your opponent controls, then this card gains 300 ATK until the end of this turn.
Once per turn, if this card would be destroyed by an opponent's card effect, you can banish 1 "Envoy of the Pope" monster from your GY instead.
You can only use each effect of "Docrates, Envoy of the Pope" once per turn.
```

Flavor: Docrates, el asesino gigante y hermano mayor de Cassios. Su cuerpo descomunal le permite ser invocado sin tributo bajo el mandato del Pope, y su fuerza bruta destruye las defensas (Spells/Traps) del enemigo antes de entrar a combate.

### Jango of the Boomerang, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: WIND
- **Type**: Warrior
- **Level**: 5
- **ATK/DEF**: 1800 / 1200

```text
If you control "Pope Ares" or a "Pope's Mandate" card, you can Special Summon this card from your hand.
Once per turn (Quick Effect): You can target 1 Level 4 or lower face-up monster your opponent controls; take control of it until the end of this turn, but its effects are negated and it cannot attack or be used as material for a Synchro, Xyz, or Link Summon.
If this card is used as material for the Xyz or Link Summon of an "Envoy of the Pope" monster: You can add 1 "Pope's Mandate" Spell/Trap from your Deck to your hand.
You can only use each effect of "Jango of the Boomerang, Envoy of the Pope" once per turn.
```

Flavor: Jango, el asesino del boomerang capaz de controlar mentalmente a sus oponentes a través de la trayectoria de su arma. Su efecto de tomar control de monstruos de bajo nivel evoca el "Boomerang Shot" del anime, que hipnotizaba temporalmente al blanco.

---

## Corrupted Gold Saints (High-Tier Envoys)

Los Gold Saints que sirvieron fielmente al Pope/Saga durante la saga del Santuario. A diferencia de sus versiones Xyz "heroicas" en `saint_cards_effects.md`, estas son **Level 8 Effect Monsters** cuya Invocación Especial requiere **Tributar 1 `Envoy of the Pope`** (pagando con Silvers/Ghosts). Su poder se amplifica mientras `Pope Ares` esté en el campo.

### Gold Saint - Deathmask of Cancer, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: DARK
- **Type**: Warrior
- **Level**: 8
- **ATK/DEF**: 2900 / 2500

```text
Cannot be Normal Summoned/Set. Must be Special Summoned (from your hand or GY) by Tributing 1 "Envoy of the Pope" monster (from your hand or field).
If this card is Special Summoned: You can banish up to 2 cards from your opponent's GY, and if you do, this card gains 300 ATK for each card banished by this effect (permanent).
Once per turn (Quick Effect): You can send 1 "Envoy of the Pope" card from your hand or Deck to the GY, then target 1 face-up monster your opponent controls; banish it.
You can only use each effect of "Gold Saint - Deathmask of Cancer, Envoy of the Pope" once per turn.
```

### Gold Saint - Aphrodite of Pisces, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: DARK
- **Type**: Warrior
- **Level**: 8
- **ATK/DEF**: 2700 / 2700

```text
Cannot be Normal Summoned/Set. Must be Special Summoned (from your hand or GY) by Tributing 1 "Envoy of the Pope" monster.
If this card is Special Summoned: Place 1 Royal Demon Rose Counter on each monster your opponent controls.
Monsters with a Royal Demon Rose Counter have their effects negated, also they lose 500 ATK for each counter on them.
During each End Phase: Place 1 Royal Demon Rose Counter on each monster your opponent controls, then destroy all monsters with 2 or more Royal Demon Rose Counters.
You can only use each effect of "Gold Saint - Aphrodite of Pisces, Envoy of the Pope" once per turn.
```

### Gold Saint - Shura of Capricorn, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: EARTH
- **Type**: Warrior
- **Level**: 8
- **ATK/DEF**: 2800 / 2100

```text
Cannot be Normal Summoned/Set. Must be Special Summoned (from your hand or GY) by Tributing 1 "Envoy of the Pope" monster.
This card's attacks cannot be negated, also this card can attack all monsters your opponent controls once each.
Once per turn (Quick Effect): You can target 1 card on the field; banish it.
While you control "Pope Ares", this card is unaffected by your opponent's Trap Card effects.
You can only use the third effect of "Gold Saint - Shura of Capricorn, Envoy of the Pope" once per turn.
```

### Gold Saint - Camus of Aquarius, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: WATER
- **Type**: Warrior
- **Level**: 8
- **ATK/DEF**: 2700 / 2600

```text
Cannot be Normal Summoned/Set. Must be Special Summoned (from your hand or GY) by Tributing 1 "Envoy of the Pope" monster.
Once per turn (Quick Effect): You can target 1 face-up monster your opponent controls; until the end of your opponent's next turn, its effects are negated, it cannot attack, cannot change its battle position, and cannot be Tributed or used as material.
While you control "Pope Ares", your opponent's monsters cannot target this card for attacks, except this one.
You can only use each effect of "Gold Saint - Camus of Aquarius, Envoy of the Pope" once per turn.
```

### Gold Saint - Aiolia of Leo, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: LIGHT
- **Type**: Beast-Warrior
- **Level**: 8
- **ATK/DEF**: 2800 / 2000

```text
Cannot be Normal Summoned/Set. Must be Special Summoned (from your hand or GY) by Tributing 1 "Envoy of the Pope" monster.
If this card is Special Summoned: You can destroy monsters your opponent controls, up to the number of "Envoy of the Pope" monsters you control.
This card can make a second attack on monsters during each Battle Phase.
If this card leaves the field by your opponent's card effect, during the next End Phase, inflict 2000 damage to your opponent.
You can only use each effect of "Gold Saint - Aiolia of Leo, Envoy of the Pope" once per turn.
```

---

## Boss Monster

### Pope Ares - Usurper of the Sanctuary
- **Card Type**: Effect Monster
- **Attribute**: DARK
- **Type**: Spellcaster
- **Level**: 10
- **ATK/DEF**: 3200 / 2800

```text
Cannot be Normal Summoned/Set.
Must be Special Summoned (from your hand or GY) by sending 2 "Envoy of the Pope" monsters you control to the GY.
If this card is Special Summoned: You can Set 1 "Pope's Mandate" Spell/Trap directly from your Deck.
Once per turn (Quick Effect): You can target 1 face-up monster your opponent controls; negate its effects, and if you do, take control of it until the End Phase, but it cannot attack directly.
While you control another "Envoy of the Pope" monster, your opponent cannot target this card with card effects.
You can only use each effect of "Pope Ares - Usurper of the Sanctuary" once per turn.
```

### Gold Saint - Saga of Gemini, Envoy of the Pope
- **Card Type**: Effect Monster
- **Attribute**: DARK
- **Type**: Warrior
- **Level**: 10
- **ATK/DEF**: 3200 / 2800

```text
Cannot be Normal Summoned/Set. Must be Special Summoned (from your hand, Deck, or GY) by Tributing 1 "Pope Ares" monster, OR when a "Pope Ares" monster you control is sent from the field to the GY by your opponent's card (in which case, Special Summon this card as a Quick Effect).
If this card is Special Summoned this way: Negate the effects of all face-up cards your opponent currently controls, and if you do, destroy them.
Once per turn (Quick Effect): You can send 1 "Envoy of the Pope" card from your hand or field to the GY; negate the activation of a card or effect, and if you do, banish that card.
This card is unaffected by the effects of non-"Envoy of the Pope" monsters.
You can only use each effect of "Gold Saint - Saga of Gemini, Envoy of the Pope" once per turn.
```

**Flavor (The True Pope Revealed)**: cuando `Pope Ares` cae del campo, su máscara se rompe y revela la verdadera identidad del usurpador: **Saga de Géminis**. No una carta separada del arquetipo — la forma final del mismo individuo. Por eso comparte el tag `Envoy of the Pope` (es, literalmente, el mismo Pope) y se invoca específicamente desde la caída de `Pope Ares`.

---

## Support Spells/Traps - Pope's Mandate

### Pope's Mandate - Chain of Command
- **Card Type**: Continuous Spell

```text
When this card is activated: You can add 1 "Envoy of the Pope" monster from your Deck to your hand.
Once per turn: You can reveal 1 "Envoy of the Pope" monster in your hand; Special Summon 1 "Envoy of the Pope" monster with a different name from your hand.
Also, for the rest of this turn after this effect resolves, you cannot Special Summon monsters from the Extra Deck, except "Saint" monsters.
If this face-up card leaves the field by an opponent's card effect: You can Special Summon 1 "Pope Ares - Usurper of the Sanctuary" from your hand or GY, ignoring its Summoning conditions.
You can only activate 1 "Pope's Mandate - Chain of Command" per turn.
```

### Pope's Guard
- **Card Type**: Quick-Play Spell

```text
When your opponent activates a card or effect that targets "Pope Ares - Voice of the Sanctuary" you control: Negate that activation, and if you do, destroy that card.
Then, you can Special Summon 1 "Envoy of the Pope" monster from your hand.
Also, for the rest of this turn, you cannot Special Summon monsters from the Extra Deck, except "Saint" monsters.
You can only activate 1 "Pope's Guard" per turn.
```

### Pope's Mandate - Extermination Order
- **Card Type**: Normal Spell

```text
Send 1 "Envoy of the Pope" monster from your Deck to the GY; add 1 "Pope's Mandate" card from your Deck to your hand, except "Pope's Mandate - Extermination Order".
If your opponent controls a Level 4 or lower monster, you can apply this effect after this card resolves.
● Special Summon 1 "Envoy of the Pope" monster from your hand.
You can only activate 1 "Pope's Mandate - Extermination Order" per turn.
```

### Pope's Mandate - Sanctuary Judgment
- **Card Type**: Quick-Play Spell

```text
Target 1 face-up monster your opponent controls; its effects are negated, also it cannot attack, until the end of this turn.
Then, if you control "Pope Ares - Usurper of the Sanctuary", you can destroy that target.
You can only activate 1 "Pope's Mandate - Sanctuary Judgment" per turn.
```

### Pope's Mandate - Silence the Rebels
- **Card Type**: Continuous Trap

```text
Your opponent cannot activate the effects of Level 4 or lower monsters on the field.
Once per turn: You can send 1 "Envoy of the Pope" card from your hand or face-up field to the GY; negate the activation of a monster effect, and if you do, destroy that monster.
```

### Pope's Mandate - Absolute Verdict
- **Card Type**: Counter Trap

```text
When your opponent activates a card or effect, while you control a "Envoy of the Pope" monster: Negate the activation, and if you do, banish that card.
If you control "Pope Ares - Usurper of the Sanctuary", your opponent cannot activate cards or effects with that banished card's original name for the rest of this Duel.
You can only activate 1 "Pope's Mandate - Absolute Verdict" per turn.
```

---

## Deck Concept (Envoy of the Pope)

El arquetipo completo se organiza en **seis tiers de poder** conectados por el tag `Envoy of the Pope`:

| Tier | Level | Cartas | Rol |
|---|---|---|---|
| 1 - Ghost Saints | L2-L4 | Geist, Astaroth, Iguana | Swarm / fodder / extender / Xyz-Link material |
| 2 - Apprentices | L4-L5 | Cassios, Shiva, Agora | Disruption ligera, protection, silencio de efectos |
| 3 - Silver Assassins | L5-L6 | Docrates, Jango | Mid beaters + steal / backrow removal |
| 4 - Silver Envoys | L6-L7 | 16 Silvers (Marin → Cepheus) | Core beatstick + disruption |
| 5 - Gold Corrupted | L8 | Deathmask, Aphrodite, Shura, Camus, Aiolia | Finishers high-tier, tributan Envoys como costo |
| 6 - Bosses | L4 / L10 / L10 | Pope Ares Voice, Pope Ares Usurper, Saga | Control absoluto del board |

- **Main Engine**: 10-14 Silver Envoys + 3-4 Ghost Saints + 4-6 Apprentices/Assassins para acceso a varios niveles.
- **Gold Package**: 4-6 Gold corruptos repartidos según matchup (Deathmask vs GY decks, Aphrodite vs boardstate, Shura vs floodgates, Camus vs OTK, Aiolia vs control).
- **Bosses**: 2-3 `Pope Ares - Voice`, 1-2 `Pope Ares - Usurper`, 1 `Saga of Gemini` (one-of revelation piece).
- **Command Package**: 8-12 cartas "Pope's Mandate".
- **Mid-Tier Utility**: `Cassios` como protector-tributable para salvar un Silver clave, `Jango` como steal contra monstruos bajos del rival, `Docrates` como beater sin-tributo bajo Pope's Mandate.

### Plan de juego

1. **Early**: abrir con `Pope Ares - Voice` + `Pope's Mandate - Chain of Command` para buscar Silvers.
2. **Mid**: usar Ghosts como costo para Tributar a un Gold corrupto (Shura/Deathmask).
3. **Late**: cuando `Pope Ares - Voice` muera, escalar a `Pope Ares - Usurper` (2 Envoys) o a `Saga of Gemini` (tributar al propio Ares) para cerrar el juego.
4. **Tech**: mantener al menos 1 Envoy en mano/GY para alimentar la cadena de tributos.

### Lore flow

```
Pope Ares (Voice) ──[fallece]──> Pope Ares (Usurper) ──[revelación]──> Saga of Gemini
   mentor simbólico                 forma agresiva                  verdadero rostro
```

Cada Gold corrupto ejecuta órdenes del Pope en una casa específica del Zodíaco. Los Silvers son la infantería del Santuario. Los Ghosts son los asesinos encubiertos. Saga, al final, **es** el Pope — y el arquetipo lo prueba mecánicamente.
